...
function mergeObjects(objects) {
  ...
  objects.forEach((currentValue) => {
    Object.keys(currentValue).forEach((key) => {
      if (!(key in sumObj)) {
        sumObj[key] = currentValue[key]; // Записываем первое значение
      } else {
        sumObj[key] += currentValue[key]; // Складываем, если число
      }
    });
  });
...
function makeWord(lettersObject) {
  const posLetterPairs = [];
  Object.entries(lettersObject).forEach(([key, value]) => {
    value.forEach((position) => {
      posLetterPairs.push([position, key]);
    });
  });
  posLetterPairs.sort((a, b) => a[0] - b[0]);
  return posLetterPairs.map((pair) => pair[1]).join('');
}
